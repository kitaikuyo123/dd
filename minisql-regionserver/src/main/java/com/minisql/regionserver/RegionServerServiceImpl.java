...
public class RegionServerServiceImpl extends RegionServerServiceGrpc.RegionServerServiceImplBase {
    ...
    /**
     * 将 SQL 中的用户表名替换为 MySQL 中的 kv_store_{regionId} 表名
     * 例如：FROM users -> FROM kv_store_USERS_718f16d2
     */
    private String convertSqlForRegion(String sql, String regionId) {
        ...
        // MySQL 中的表名格式：kv_store_{regionId}
        // regionId 可能包含特殊字符，需要替换为合法字符
        String mysqlTableName = "kv_store_" + regionId.replaceAll("[^a-zA-Z0-9_]", "_");
        ...
        // 2. 处理 UPDATE 表名
        result = result.replaceAll(
            "(?i)\\bUPDATE\\s+`?([a-zA-Z_][a-zA-Z0-9_]*)`?\\s+",
        ...
        // 4. 处理 FROM 表名（SELECT 等）
        result = result.replaceAll(
            "(?i)\\bFROM\\s+`?([a-zA-Z_][a-zA-Z0-9_]*)`?(\\s|$|,|WHERE|ORDER|GROUP|HAVING|LIMIT|JOIN|ON|\\)|;)",
            "FROM `" + mysqlTableName + "`$2"
        );

        // 5. 处理 JOIN 表名
        result = result.replaceAll(
            "(?i)\\bJOIN\\s+`?([a-zA-Z_][a-zA-Z0-9_]*)`?\\s+",
            "JOIN `" + mysqlTableName + "` "
        );
